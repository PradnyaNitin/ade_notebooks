...
-- MAGIC %md
-- MAGIC create catalog sql_catalog;
-- MAGIC
-- MAGIC use sql_catalog;  

-- COMMAND ----------

-- MAGIC %md
-- MAGIC create schema sql_schema;
-- MAGIC
...
USE CATALOG sql_catalog;

create schema sql_schema;

-- COMMAND ----------

show schemas in sql_catalog;
...
select empno,ename,sal,round (sal/30, 2),sal *12 as AnnualSal from emp order by annualsal asc;
...
select empno,ename,job, hiredate,round(months_between(current_date,hiredate),00) as exp_in_months from emp ;
...
select emp.*, round( months_between(current_date, hiredate)/12 , 2) as Exp_in_yr ,
round((sal/30),2)as Daily_sal from emp
where (sal/30) > 100;
